...
* **目的**: 行動送信 → 1 ターン進行を待機。
* **使用技術**: 非同期待機 (`asyncio.sleep`), Battle.turn 監視
* **達成条件**:

  * キューに action 投入後、最新 `rqid` の `request` を処理し `battle.turn` 増加
    または終了を待つ
  * 強制交代用に複数 `request` が来ても対応できること
  * タイムアウト設計。
* **テスト**: 強制交代を含むターンでも正常に進行し、毎 step ごとに turn が +1。
