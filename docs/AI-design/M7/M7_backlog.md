...
| R-2 | `撃破/被撃破 ±1` 実装           | `rewards/knockout.py`      | エージェント側・敵側の faint 検知ロジック追加。ログに内訳出力。 |
| R-3 | 報酬合成マネージャ              | `rewards/composite.py`, `configs/reward.yaml` | YAML で各係数を切替。TensorBoard で sub-reward 値をプロット。 |
