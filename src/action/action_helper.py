...
def get_available_actions_with_details(battle: Battle) -> Tuple[np.ndarray, Dict[int, ActionDetail]]:
    ...
    for action_idx, (action_type, sub_idx) in mapping.items():
        ...
        elif action_type == 'terastal' and sub_idx < len(moves_sorted) and battle.can_tera:
            mv = moves_sorted[sub_idx]
            detailed[action_idx] = {
                'type': 'terastal',
...
def action_index_to_order(player: Player, battle: Battle, action_index: int) -> BattleOrder:
    '''
    ...
    if action_index not in mapping:
        raise ValueError(f'Invalid or unavailable action index: {action_index}. Available: {mapping}')
