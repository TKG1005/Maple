...
class RuleBasedPlayer(Player):
    ...
    def choose_move(self, battle: Battle):
        ...
        if my_active_pokemon:
            print(
                f"My Active: {my_active_pokemon.species} (HP: {my_active_pokemon.current_hp_fraction * 100:.1f}%)"
            )
        if opponent_active_pokemon:
            print(
                f"Opponent Active: {opponent_active_pokemon.species} (HP: {opponent_active_pokemon.current_hp_fraction * 100:.1f}%)"
            )
        # --- 状態観測 ---
        try:
            _ = self.observer.observe(battle)  # 必要であればコメントアウト解除
            # print(f"State observed. Vector length: {len(_)}")  # 詳細すぎる場合はコメントアウト
        ...
        # --- 行動取得 ---
        try:
            available_action_mask, available_action_mapping = get_available_actions(
                battle
            )
            ...
            if not available_action_mapping:  # マッピングが空なら選択肢なし
                print(
                    "LOG: No actions available in mapping. Choosing a random move (fallback)."
                )
                return self.choose_random_move(battle)
        except Exception as e:
        ...
        # --- ルールベースの意思決定 ---
        if (
            opponent_active_pokemon and opponent_active_pokemon.type_1
        ):  # 相手ポケモンとそのタイプが判明している場合
            ...
            for action_idx, (
                action_type,
                original_move_idx,
            ) in available_action_mapping.items():
                if (
                    action_type == "move"
                ):  # 通常技のみ考慮 (テラスタルは別途ルール化が必要なら追加)
                    if not available_action_mask[
                        action_idx
                    ]:  # マスクで不可ならスキップ
                        continue
        ...
        if chosen_action_index == -1:
            possible_action_indices = [
                idx
                for idx in available_action_mapping.keys()
                if available_action_mask[idx]  # マスクが1のものを選択
            ]

            if not possible_action_indices:  # 利用可能な行動が本当にない場合
                print(
                    f"LOG: ({applied_rule}) -> No valid actions available based on mask and mapping. Choosing random move (ultimate fallback)."
                )
                return self.choose_random_move(battle)

            chosen_action_index = np.random.choice(possible_action_indices)
            applied_rule += (
                f" -> Fallback to Random (from {len(possible_action_indices)} options)"
            )
            # どの行動がランダムで選ばれたかを追加でログ
            randomly_selected_action_details = available_action_mapping.get(
                chosen_action_index
            )
        ...
        try:
            ...
            action_type_log, original_idx_log = available_action_mapping[
                chosen_action_index
            ]
            action_details_log = ""
            if action_type_log == "move":
                action_details_log = (
                    f"Move: {battle.available_moves[original_idx_log].id}"
                )
            elif action_type_log == "terastal":
                action_details_log = (
                    f"Terastal Move: {battle.available_moves[original_idx_log].id}"
                )
            elif action_type_log == "switch":
                action_details_log = (
                    f"Switch: {battle.available_switches[original_idx_log].species}"
                )

            print(f"LOG: Rule Applied: '{applied_rule}'")
            print(
                f"LOG: Chosen Action Index: {chosen_action_index} ({action_details_log})"
            )
            ...
            KeyError
        ):  # chosen_action_index がマッピングにない場合（基本的には起こらないはず）
            print(
                f"LOG: Error - Chosen action index {chosen_action_index} not in available_action_mapping. This should not happen."
            )
            ...
            print(
                f"LOG: Error converting action index {chosen_action_index} to order: {e}"
            )
    ...
    def teampreview(self, battle: Battle):
        ...
        if num_to_select > 0:
            ...
            # チームが空などのエッジケース
            print(
                "LOG: Warning: Team is empty or invalid during teampreview. Defaulting to /team 123"
            )
            return "/team 123"
...
if __name__ == "__main__":
    print(
        "RuleBasedPlayer class defined with super effective move logic (import fixed)."
    )
    # 対戦実行スクリプトでテストしてください。
