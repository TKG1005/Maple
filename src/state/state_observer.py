...
class StateObserver:
    ...
    def observe(self, battle: AbstractBattle) -> np.ndarray:
        ...
        if battle is None:
            ...
            raise ValueError("StateObserver.observe() called with None battle object, which is not supported for actual observation generation.")
        ...
        for group, features in self.spec.items():
            for key, meta in features.items():
                ...
                enc_func = self.encoders.get((group, key), lambda x: [float(x)] if not isinstance(x, list) else [float(i) for i in x]) # デフォルトエンコーダもリストを返すように
                ...
                state.extend(encoded_val if isinstance(encoded_val, list) else [encoded_val])
        ...
        for group, features in self.spec.items():
            for key, meta in features.items():
                ...
                if encoder_func is None:
                    ...
                    print(f"Warning: No specific encoder found for {group}.{key}. Assuming 1 dimension.")
                    dimension += 1
                    continue
                ...
                try:
                    ...
                    default_value_for_test = eval(raw_default) if isinstance(raw_default, str) else raw_default
                except Exception:
                ...
                if encoder_type == 'onehot':
                    ...
                    if isinstance(default_value_for_test, list): # デフォルトが既にリストならその長さを採用できるが、エンコーダを通す方が確実
                        test_input = default_value_for_test # これをエンコーダがどう扱うか
                    ...
                    try:
                        test_input = float(default_value_for_test if not isinstance(default_value_for_test, list) else 0)
                ...
                try:
                    ...
                    print(f"Error while testing encoder for {group}.{key} with input '{test_input}': {e}")
                    ...
                    if meta.get('encoder') == 'onehot' and meta.get('classes'):
                         ...
                         dimension += 1
                    print(f"Warning: Assuming 1 dimension for {group}.{key} due to encoder test error.")
        
        if dimension == 0:
            raise ValueError("Calculated observation dimension is 0. Check state_spec.yml and StateObserver.get_observation_dimension().")
        return dimension
    ...
    def _build_context(self, battle: AbstractBattle) -> dict:
        ...
        active = next((p for p in my_team if p.active), None) # None の場合のデフォルトを追加
        ctx['active'] = active
        ctx['active_sorted_moves'] = sorted(active.moves.values(), key=lambda m: m.id) if active and active.moves else []

        bench = [p for p in my_team if not p.active] if active else my_team # activeがNoneなら全員bench扱い(要件次第)
        ...
        opp_active = next((p for p in opp_team if p.active), None) # None の場合のデフォルトを追加
    ...
    def _extract(self, path: str, ctx: dict, default):
        ...
        try:
            ...
            if not path:
                return default
            return eval(path, {"PokemonType": PokemonType, "MoveCategory": MoveCategory, "AbstractBattle": AbstractBattle}, ctx) # Enum型をevalのスコープに追加
    ...
    def _build_encoders(self, spec: dict):
        ...
        for group, features in spec.items():
            for key, meta in features.items():
                ...
                if kind == 'identity':
                    # 出力をリスト[float]に統一
                    enc[enc_key] = lambda x, d=raw_default_for_lambda: [float(x if x is not None else eval(d) if isinstance(d,str) else d)]
                elif kind == 'onehot':
                    classes = meta.get('classes', [])
                    # _onehot_encoder_simple を呼び出すlambda
                    enc[enc_key] = lambda val, cls_list=classes, default_str=raw_default_for_lambda: self._onehot_encoder_simple(val, cls_list, default_str)
                elif kind == 'linear_scale':
                    lo, hi = meta.get('range', [0, 1])
                    # evalできる形式のデフォルト値文字列を期待
                    enc[enc_key] = lambda x, l=lo, h=hi, d_str=raw_default_for_lambda: [((float(x) - l) / (h - l)) if x is not None and h > l else (float(eval(d_str)) if isinstance(d_str,str) else float(d_str))]
    ...
    def _onehot_encoder_simple(self, val, cls_list: list, default_str_val: str) -> list:
        ...
        elif hasattr(val, 'name') and isinstance(val.name, str): # 他のEnumっぽいオブジェクト
            value_to_encode_str = val.name.lower()
        else:
        ...
        encoded_list = [1.0 if value_to_encode_str == str(c).lower() else 0.0 for c in cls_list]
